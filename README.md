# A Pipeline for Fusing Causal Beliefs from Multiple Sources under Uncertainty

This repository provides the data, code, fusion outputs, benchmark metrics, and figures used to evaluate uncertainty aware fusion methods for causal edge beliefs. The benchmark focuses on elicited beliefs over candidate directed causal edges from multiple heterogeneous sources, where each source assigns belief masses to three hypotheses: edge existence, edge non-existence, and explicit uncertainty.

## Repository Structure

```text
A-Benchmark-for-Fusing-Elicited-Causal-Beliefs-under-Uncertainty/
│
├── README.md
...
The benchmark is constructed from structured probability assignments over candidate directed causal edges.

For each candidate edge, each source provides three belief masses:
...
This folder contains the original expert-assigned spreadsheet files. Each spreadsheet corresponds to one expert and contains belief assignments for the same set of candidate directed edges.

### `data/edge_level_expert_beliefs/`

This folder contains edge-level source belief tables. Each file corresponds to a candidate edge and includes the belief assignments provided by all experts for that edge.
